...
export const createTournament = async (req, res) => {
  ...
  const { error } = tournamentValidation(newReqBody, "POST");
  if (error) {
...
const tournamentJoiSchema = Joi.object({
  game: Joi.string().trim().required().min(4),
...
export const updateTournament = async (req, res) => {
  const newReqBody = emptyDataValidation(req.body);
  const { error } = tournamentValidation(newReqBody, "PUT");
  if (error) {
    return res.status(400).json({ message: error.details[0].message });
  }
  try {
